...
\[
    ||F_q((\tetaw)_i)-F_p(\h_i)||_\text{TV}\leq\sqrt{\frac{1}{2}\KL{\priorw}{\qw}},
\]
где $||\cdot||_\text{TV}$ --- расстояние по вариации, $F_q, F_p$ --- функции распределения  $\qw,\priorw,$ дивергенция $\KL{\priorw}{\qw}$ берется при фиксированной структуре $\G$.
...
\]
Т.к. по неравенству Йенсена 
\[
    \int_{\G} ||F_q({\tetaw[i]})-F_p(\h_i)||^2_\text{TV}\qG[][{\tetaG[i]}] d\G \geq  
\]
\[
\geq \left(\int_{\G} ||F_q({\tetaw[i]})-F_p(\h_i)||_\text{TV}\qG[][{\tetaG[i]}] d\G\right)^2,
\]
то $$\lim_{i \to \infty} \int_{\G}||F_q({\tetaw[i]})-F_p(\h_i)||_\text{TV} \qG[][{\tetaG[i]}] d\G = 0.$$
...
Пусть:
\begin{enumerate}[label={\arabic*)}] 
\item Задано непустое множество непрерывных по параметрам распределений на структуре $\mathbf{P}$, чьи плотности не принимают нулевое значение, где хотя бы одно распределение $p_1 \in \mathbf{H}$ является Gumbel-Softmax распределением и для любого значения $\s \in \Uh, \lamT \in \Ulam$ существует значение параметров распределения $p_1$, такое что $p_1 = \priorG$.
