...
Пусть:
\begin{enumerate}[label={\arabic*)}] 
\item задано непустое множество непрерывных по параметрам распределений на структуре $\mathbf{P}$, не принимающих нулевое значение, где хотя бы одно распределение $p_1 \in \mathbf{H}$ является Gumbel-Softmax распределением и для любого значения $\s \in \Uh, \lamT \in \Ulam$ существует значение параметров распределения $p_1$, такое что $p_1 = \priorG$.
...
\]
\item Существуют значения метапараметров $\lamCQ,\lamLL, \lambda_1, \lambda_2, \lamT,$ такие что существуют гиперпараметры $\h_1, \h_2 \in \Uh$:
\[
...
Пусть 
\[
K_2 =  \max_{\h \in \Uh} \log \priorh  - \min_{\h \in \Uh} \log \priorh + \max_{\h \in \Uh} \min_{\teta \in \Uteta} \KL{\q}{\prior} - 
\]
\[
\min_{\h \in \Uh, \teta \in \Uteta} \KL{\q}{\prior} + \max_{\teta \in \Uteta} \frac{1}{\lamCL} \E_{\q}\log \LL - 
\]
...
\]
оценим снизу, а также добавим и вычтем $\KL{\q[\teta_2]}{\prior[][][\h_2]}$
...
\]
сведем выражение до $\Val$
\[
= \Val[\h_1] - \Val[\h_2] - \log\priorh[\h_1] + \log\priorh[\h_2]  +
\]
\[
+  \min_{\h}\KL{\q[\teta_2]}{\prior}  -  \KL{\q[\teta_2]}{\prior[][][\h_2]} >
\]
воспользуемся неравенством $\Val[\h_1] - \Val[\h_2]>K_2$
\[
> K_2 - \log\priorh[\h_1] + \log \priorh[\h_2] + \min_{\h}\KL{\q[\teta_1]}{\prior} 
\]
\[
...
Рассмотрим разность:
\[
\min_{\h}\KL{\q[\teta_2]}{\prior}  -  \KL{\q[\teta_1]}{\prior[][][\h_2]}  =
\]  
т.к. $\teta_1$ --- решение нижней задачи оптимизации:
\[
\min_{\h}\KL{\q[\teta_2]}{\prior}  - \frac{1}{\lamCL}\E_{\q[\teta_1]}\log \LL + 
\]
\[
...
\]
\[
    \max_{\teta}\left(\min_{\teta'} \frac{1}{\lamCL} \E_{\q[\teta']} \log \LL - \KL{\q}{\priorh[\h_1]} \right) \geq
\]
...
\]
\[
\min_{\teta} \frac{1}{\lamCL} \E_{\q} \log \LL - \min_{\teta}\KL{\q}{\prior[\h_1]} \geq
\]
...
$$ \Val = \E_{\q} \log \LL - \KL{\q}{\prior} + \log\priorh.$$
...
\[
    p_1 = \prior[\h_1].
\]
Для остальных распределений $p' \in \mathbf{P}$ положим коэффициент $\lambda^{Q}_\text{struct} \in \lamS$ равным нулю.  
...
Слагаемое $\E_{\q}\log\LL$ не зависит от метапараметров $\lambda$. Слагаемое $\log\priorh$ непрерывно по метапараметрам по свойству обратного гамма-распределения.

Достаточным условием непрерывности функций вида $\KL{p_1}{p_2}$ является непрерывность по метапараметрам функций $p_1 (\log{p_1} - \log{p_2})$  почти всюду и ограниченность интегрируемой функцией.  Априорные распределения задаются нерперывными функциями плотности $\priorw, \priorG$, не принимающими нулевое значение, и являющимися непрерывными по метапараметрам. Функция $\q$ принимает нулевое значение лишь в конечном количество точек.  Функция $\q (\log \q - \log \prior)$ ограничена на компакте $\Ulam$, поэтому слагаемое $\KL{\q}{\prior}$ является непрерывным по метапараметрам.
Выражения вида $\prior (\log \priorG - \log p), p \in \mathbf{P}$ также являются непрерывными по метапараметрам и ограниченными, поэтому слагаемые вида $\KL{\priorG}{p}$ являются непрерывными. Поэтому функции $\Loss, \Val$ являются непрерывными по метапараметрам, что и требовалось доказать.
\end{proof}
...
\[
    -\lamCQ \KL{\q}{\prior}. 
\]
Параметры $\teta$ вариационного распределения $\q$ не зависят от слагаемых вида $\log \priorh$ и $\KL{\prior}{p'}, p' \in \mathbf{P},$ поэтому нижняя задача оптимизации
...
\]
\[
-\sum_{p', \lambda \in \mathbf{P}, \lamS} \KL{\priorG}{p'} + \log \priorh \to \max_{\teta}
\]
