...
\begin{theorem}
Пусть $\boldsymbol{\theta}_1,\boldsymbol{\theta}_2,\dots$ --- бесконечная последовательность векторов вариационных параметров, такая что $\lim_{i \to \infty}C_p(\boldsymbol{\theta}_i) = 0$, $\lim \sigma_q  \neq 0, \lim \theta_q \neq 0.$ Тогда относительная плотность вариационных параметров данной последовательности стремится к единице:
\[
...
Первое слагаемое равняется нулю только в случае, когда априорное распределение параметров полностью совпадает с вариационным. Т.к. предел $\sigma_q \neq 0$, то предельным распределением параметров будет нормальное распределение с нулевым средним. 
Отсюда относительная плотность параметров будет равняться нулю. Заметим, что условие устремления параметрической сложности к нулю является теоретическим. Предложенная в данной работе модель достигает этого условия только в вырожденоом случае.

\end{proof}
Теорема утверждает, что при устремлении параметрической сложности моделей к нулю, параметры модели станоятся неинформативными и подлежащими удалению. Заметим, что в случае $\lim \sigma_q = 0$ модель имеет вырожденные признаки, которые подлежат замене на константу. 
