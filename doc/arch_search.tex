...
\[
   \lim_{i \to \infty} \E_{\qG[(\tetaG)_i]} {\rho}(\w|\G, (\tetaw)_i, (\h)_i, \lam) = 1.
\]
...
\[
    ||F_q((\tetaw)_i)-F_p(\h_i)||_\text{TV},\leq\sqrt{2\log{\qw[][][(\tetaw)_i]} - \log{\priorw[][][\h_i]}d\w} = 0,
\]
...
\]
представляя единицу как $\frac{\qw[\text{mode}~\priorw]}{\qw[\text{mode}~\priorw]]}$
\[
...
\]
используя липшецевость функции $\qw$
\[
...
\]
по неравенству треугольника и используя выражение $\w = \w + \boldsymbol{\nu}(t) -\boldsymbol{\nu}(t)$
\begin{equation}
\label{eq:tmp}
    \leq    \iint_{\G,\w} |\w -\boldsymbol{\nu}(t)||\priorw -\qw|\qG d\w d\G   +
\end{equation}
...
\]
\[
    - \sum_{p',\lambda \in \mathbf{P},\lamS} \lambda\KL{\q[\teta^{*}]}{p'} + \log\priorh, 
\]
\begin{equation}
\tag{$L^{*}$}
...
\]
где $\mathbf{P}$ --- непустое множество распределений на структуре $\G$, $\lamCQ, \lamCL, \lamS$  --- некоторые числа. Множество распределений $\mathbf{P}$ отвечает за перебор структур $\G$ в процессе оптимизации модели. Более подробное объяснение данного множества дано ниже. 
...
\item Вариационное распределение $\q$ является  абсолютно непрерывным, плотность которого непрерывна по метапараметрам $\boldsymbol{\lambda}$.

\item Задан компакт  $U = \Utetaw \times \UtetaG \times \Uh \times \Ulam,$ где параметры распределений $\mathbf{P} \in \Ulam$.
...
\item Существует значение метапараметров $\lambda_1, \lambda_2, \lamLL$, такое что
\[
\max_{\h \in \Uh} \log \priorh -\min_{\h \in \Uh}\log \priorh < \max_{\h \in \Uh} \Val - \min_{\h \in \Uh} \Val
\] 
при $\lamS = \mathbf{0}, \lamCQ = 0$.

\item Существует значение метапараметров $ \lamCQ, \lambda_1, \lambda_2, \lamT$, такое что 
\[
    \max_{\h \in \Uh} \log  \priorh - \min_{\h \in \Uh} \log  \priorh + \max_{\h \in \Uh} \min_{\teta \in \Uteta} \KL{\q}{\prior} -
\]
\[ \min_{\h \in \Uh, \teta \in \Uteta}  \KL_{\q}{\prior} + \max_{\teta \in \Uteta}\frac{1}{\lamCL}\E_{\q} \log \LL - 
\]
\[
...
\item Существуют значения метапараметров $\lamCQ,\lamLL, \lambda_1, \lambda_2, \lamT,$ такие что 
\[
\max_{\h_1,\h_2 \in \Uh} \KL{\prior[][][\h_1]}{\prior[][][\h_2]} -
\]
\[
- \min_{\h_1,\h_2 \in \Uh} \KL{\prior[][][\h_1]}{\prior[][][\h_2]}< 
\]
\[
...
\]
Пусть $\h_1, \h_2 \in \Uh, \Val[\h_1] - \Val[\h_2]>K_1$.
Тогда 
\[
...
Докажем критерий 4. 
Пусть $\lam$ удовлетворяют восьмому условию и $\lamLL = 0, \lamS = \mathbf{0}$.
Пусть 
\[
K_2 =  \max_{\h \in \Uh} \log \priorh  - \min{\h \in \Uh} \log \priorh + \max_{\h \in \Uh} \min_{\teta \in \Uteta} \KL{\q}{\prior} - 
\]
\[
...
\[
\geq    \min_{\h \in \Uh} \KL{\q[][][\teta_2]}{\prior} - \min_{\h \in \Uh}\KL{\q[][][\teta_1]}{\prior} + \KL{\q[][][\teta_2]}{\prior[][][\h_2]} - \KL{\q[][][\teta_2]}{\prior[][][\h_2]} = 
\]
\[
= \Val[\h_1] - \Val[\h_2] - \priorh[\h_1] + \priorh[\h_2]  +
\]
\[
+ \min_{\h}  \min_{\h}\KL{\q[][][\teta_2]}{\prior}  -  \KL{\q[][][\teta_2]}{\prior[][][\h_2]} >
\]
\[
> K_2 - \log\priorh[\h_1] + \log \priorh[\h_2] + \min_{\h}  \min_{\h}\KL{\q[][][\teta_1]}{\prior}  -  \KL{\q[][][\teta_1]}{\prior[][][\h_2]}.
\]
Рассмотрим разность:
\[
\min_{\h}\KL{\q[][][\teta_1]}{\prior}  -  \KL{\q[][][\teta_1]}{\prior[][][\h_2]}  =
\]  
\[
\min_{\h}\KL{\q[][][\teta_1]}{\prior}  - \frac{1}{\lamCL}\E_{\q[\teta_1]}\log \LL + 
\]
\[
\max_{\teta} (\frac{1}{\lamCL} \E_{\q}\log \LL - \KL{\q}{\priorh[\h_1]}) \geq
\]
% взяли максимум от правдоподобия
\[
\geq \min_{\h}\KL{\q[][][\teta_1]}{\prior} - \max_{\teta}\frac{1}{\lamCL}\E_{q} \log \LL + 
\]
\[
    \max_{\teta} \min_{\teta'} \frac{1}{\lamLL} \E_{\q[\teta']} \log \LL - \KL{\q}{\priorh[\h_1]} \geq
\]
\[
\geq \min_{\teta,\h} \KL{\q}{\prior} - \max_{\teta}\frac{1}{\lamCL}\E_{\q} \log \LL +
\]
\[
\min_{\teta} \frac{1}{\lamCL} \E_{\q} \log \LL - \min{\teta}\KL{\q}{\prior[\h_1]} \geq
\]
...
\]
\[
+ \min_{\teta} \frac{1}{\lamCL}\E_{q} \log \LL - \max_{\h}\min{\teta}\KL{\q}{\prior}.
\]
Складывая полученную оценку с $K_2 -\log \priorh[\h_2] + \log \priorh[\h_1]$ получаем разность параметрических сложностей больше нуля.

Докажем критерий 5. Пусть $\lamCQ = \lamCL = \lamLL > 0,$ $\lamS = \mathbf{0}.$ Тогда функции $\Loss$ и $\Val$  можно записать как: 
$$\Loss = \Val \propto $$ $$\E_{\q} \log \LL - \KL{\q}{\prior},$$ что и требовалось доказать.
...
Без ограничения общности предположим, что $\Val[\h_1] - \Val[\h_2]>0$ при $\lam.$ Также без ограничения общности будем полгаать, что множестве $\mathbf{P}$ состоит только из одного распределения на структуре $\G$, равного распределению на структуре $\priorG[][\h_1].$
...
Положим для $\lam'$ параметр $\lamS$ равным максимальному значению: $\lamS = \max {\lamS}'$.
Тогда при $\lam'$ неравенство 
\[
\Val[\h_1] = \Val[\h_2] + {\lamS}' K_4 = \Val[\h_1]-\Val[\h_2] + \max_{\h} \Val - \min_{\h} \Val = 0,
\]
что и требовалось доказать.

Докажем критерий 7. Достаточным условием непрерывности функций $\Loss$, $\Val$ является непрерывность входящих в нее слагаемых. 
Т.к. априорные распределения задаются нерперывными функциями плотности $\priorw, \priorG$, и функция плотности $\priorG$ распределения структуры $\G$ ограничена на компакте, то дивергенция $\KL{\q}{\prior}$ непрерывна по метапараметрам.
Т.к. остальные слагаемые функций оптимизаций $\Loss,\Val$ также непрерывны по метапараметрам, то непрерывна и сами функции оптимизации.
\end{proof}

Метапараметрами данной задачи~\eqref{eq:qopt} являются коэффициенты $\lamCL, \lamCQ$, отвечающие за регуляризацию верхней и нижней задачи оптимизации, коэффициент $\lamLL$ отвечает за максимизацию правдоподобия, а также параметры распрделений $\mathbf{P}$ и вектор коэффициентов перед ними $\lamS$. 

В предельном случае, когда температура $\lamT$ близка к нулю, а множество $\mathbf{P}$ состоит из распределений, близких к дискретным,а соответствующим всем возможным структурам, калибровка $\lamS$ порождает последовательность задач оптимизаций, схожую с перебором структур. Рассмотрим следующий пример. 

\begin{example} 
Рассмотрим вырожденный случай поведения функции $\Val$, когда $\lamLL = \lamCQ = 0$. Пусть модель использует один структурный параметр, в качестве априорного распределения на структуре задано распределение Gumbel-Softmax с $\lamT$. Пусть в качестве множества распределений $\mathbf{P}$ используется два распределения Gumbel-Softmax, сконцентрированных близко к вершинам симплекса:
\[
...
\[
    \frac{1}{m}\Loss \approx  \E_{y,\x} \E_{\q} \log \LL -\frac{\lamCL}{m}\KL{\q}{\prior}.
\]
...
\]
\[
= \E_{y,\x} \E_{\q} \log \LL  -\frac{\lamCL}{m}\KL_{\q}{\prior}.
\]
...
\begin{theorem}
Пусть ${\lamCQ} = \lamCL=\lamLL, \lamS=\mathbf{0}$. Тогда:
\begin{enumerate}
...
\vspace{-0.3cm}
\[
    \E_{\q}\log \LL - \KL{\q}{\prior} + \log \prior \to \max_{\h}.
\]
...
\[
\log \EV  = \E_{\q} \log\frac{\LL}{\prior} + \KL{\q}{\prior} = 
 \]
\[
=\E_{\q} \log \LL - \KL{\q}{\prior} + \KL{\q}{\post}.
\]
...
\]
\[
-\sum_{p', \lambda \in \mathbf{P}, \lamS} \KL{\priorG}{p'} - \log \priorh \to \max_{\h, \teta}. 
\]
\end{theorem}
\begin{proof}
Параметры вариационного распределения $\q$ не зависят от слагаемых вида $\log \priorh$ и $\KL{\prior}{p'}, p' \in \mathbf{P},$ поэтому нижняя задача оптимизации:
\[
    \E_{\q} \log \LL  -
\]
\[
    \lamCL \KL{\q}{\prior}  \to \max_{\teta}
\]
