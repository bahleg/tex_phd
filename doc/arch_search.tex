...
\item Параметры модели $\w$ имеют конечные вторые моменты по маргинальным распределениям $\qw$:
\[
    \E_\q \w^2 = \E_{\qG} \E_{\qw} \w^2 < \infty;
\]
\[
    \E_{\prior} \w^2 = \E_{\priorG} \E_{\priorw} w^2 < \infty.
\]
...
\[
   \E_{\qG} \boldsymbol{\rho}(\w|\G, \tetaw, \h, \lam)^{-1} \to 1.
\]
...
\[
    ||F_q(\teta)-F_p(\h)||_\text{TV},\leq\sqrt{2\KL{\q}{\prior}},
\]
где $||\cdot||_\text{TV}$ --- расстояние по вариации, $F_q, F_p$ --- функции распределения   $\q,\prior.$
Т.к. дивергенция состоит из двух неотрицательных величин, то обе они стремятся к нулю.
Рассотрим вторую величину:
\[
   \lim 0 = \KL{\qw,\priorw} = |\int_{\G} \KL{\qw,\priorw} \geq \int_{\G} ||F_q - F_p||_\text{TV}d\G.  
\]
Отсюда $ \lim_{i \to \infty} ||F_q(\teta)-F_p(\h)||_\text{TV} = 0.$
По теореме Шеффе данное выражение можно переписать как:
\[
    \lim \frac{1}{2}\iint_{\w,\G} |\priorw - \qw| d\G d\w = 0.
\]

Для произвольного $\teta_j$ рассмотрим выражение:
\[
    |\int_{\G} \frac{q(\E \w)}{q(\mathbf{M})} -  \frac{q(\E \w)}{{q(\mathbf{M})}}| \leq \int_{\G} |\frac{q_j(\E \w)}{{q(\mathbf{M})}} -  \frac{q(\E \w)}{{q(\mathbf{M})}}| \leq \frac{\max{L}}{\min q(\mathbf{M})}\int_{\G}  |\E \w - \E \w|  \leq
\]
\[
    \leq L\int_{\G} |\w| |\q - \prior|.
\]

%https://math.stackexchange.com/questions/112786/convergence-in-law-and-uniformly-integrability
Т.к. вторые моменты $\E_{\qw} \w^2, \E_{\priorw} \w^2$ конечны, то случайная величина $\w$ равномерно интегрируема как при маргинальном распределении $\qw$, так и при маргинальном распределении $\priorw$.
...
\[
    \E|\w - \boldsymbol{\nu}(t)| \leq \varepsilon.
\]
где матожидание берется по  распределениям  $\qw$ и $\priorw$.
Тогда 
\[
    \int_{\G} |\E_{\qw} \w-\E_{\priorw} \w| \leq     \int_{\G}  \int_{\w} |\w -\boldsymbol{\nu}(t)||\priorw-\qw|d\w + 
\]
\[
 +    \int_{\G}  \int_{\w} |\boldsymbol{\nu}(t)||\priorw-\qw|d\w \leq
\]
\[
    \leq     \int_{\G}  \int_{\w} |\w -\boldsymbol{\nu}(t)|\priorw d\w +    \int_{\G}  \int_{\w} |\w -\boldsymbol{\nu}(t)|\qw|d\w  +
\]
\[ +      \int_{\G}  \int_{\w} |\boldsymbol{\nu}(t)|\priorw - \qw|d\w.
\]
Обозначим за $\h_i$ --- решение задачи~\eqref{eq:dkl_solve} для вектора $\teta_i$.
Т.к. $|\boldsymbol{\nu}(t)|$ --- ограничена, то 
\[
      \int_{\G}  \int_{\w} |\boldsymbol{\nu}(t)|\priorw - \qw \leq t     \int_{\G}  \int_{\w} |\priorw - \qw|d\w =
\]
\[
= t    \int_{\boldsymbol{\Gamma}}  \int_{\w} |\prior - \q|d\G d\w \to_{i \to \infty} 0.
\]
\[
   \lim_{i \to \infty}     \int_{\G}  |\E_{\qw}\w-\E_{\priorw}\w| \leq      \int_{\G}  \int_{\w} |\w -\boldsymbol{\nu}(t)||\priorw-\qw|d\w 
\]
для любого $t$. Устремляя $t$ к бесконечности, получим $ \lim_{i \to \infty}     \int_{\G}  |\E_{\qw}\w-\E_{\priorw}\w|  = 0.$
Таким образом, мода $\qw$  стремится в среднем к моде априорного распределения $\mathbf{M}$.
\end{proof}
