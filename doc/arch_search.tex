...
Сформулируем и докажем теорему о связи относительной плотности и параметрической сложности модели. Перед доказательством теоремой докажем две вспомогательные леммы.
...
\]
\textit{представляя единицу как }$\frac{\qw[\text{mode}~\priorw]}{\qw[\text{mode}~\priorw]}$
\[
...
\]
где $\teta[i] = [\tetaw[i], \tetaG[i]],$ $\h[i]$ --- решение задачи~\eqref{eq:dkl_solve} для $\teta[i]$.
...
По условию дивергенция~\eqref{eq:kl_full} стремится к нулю при $i \to \infty$. Она декомпозируется на два  неотрицательных слагаемых, поэтому обе они стремятся к нулю.
...
%\end{center}
\[
 \lim_{i \to \infty} \left|\int_{\G} \int_{\w} \log \left(\frac{\qw[][][{\tetaw[i]}]}{\priorw[][][{\h[i]}]}\right)\qG[][{\tetaG[i]}]\qw[][][{\tetaw[i]}] d\w d\w \right| \geq  
\]
%\begin{center}
\textit{по неравенству Пинскера}
%\end{center}
\[
\geq \lim_{i \to \infty}  \int_{\G} ||F_q({\tetaw[i]})-F_p(\h_i)||_\text{TV}\qG[][{\tetaG[i]}] d\G \geq 0.  
\]
Отсюда $$\lim_{i \to \infty} \int_{\G}||F_q({\tetaw[i]})-F_p(\h_i)||_\text{TV} \qG[][{\tetaG[i]}] d\G = 0.$$
...
$$
    \lim_{i \to \infty} \iint_{\G,\w} |\w| |\qw - \priorw[][][\h_i]|\qG[][{\tetaG[i]}] d\w d\G  =
$$
\textit{добавим предел по }$t$\textit{, от которого не зависит данное выражение}
$$
= \lim_{t \to \infty}\lim_{i \to \infty}\iint_{\G,\w} |\w| |\qw[][][{\tetaw[i]}] - \priorw[][][{\h[i]}]|\qG[][{\tetaG[i]}] d\w d\G \leq
$$
\textit{из выше написанных неравенств}
$$
    \lim_{t \to \infty}\lim_{i \to \infty} \iint_{\G,\w} |\w -\boldsymbol{\nu}(t)||\priorw[][][{\h[i]}] -\qw[][][{\tetaw[i]}]| d\w d\G   +
$$
$$
 + \iint_{\G,\w} |\boldsymbol{\nu}(t)| |\qw[][][{\tetaw[i]}] - \priorw[][][{\h[i]}]|\qG[][{\tetaG[i]}] d\w d\G \leq
$$
$$
...
\item Критерий перебора оптимальных структур: существует константа $K_3>0$, такая что существует хотя бы одна пара гиперпараметров $\h_1, \h_2 \in \Uh:$
...
\item Существует значение метапараметров $ \lamCQ>0, \lambda_1>0, \lambda_2>0, \lamT>0 \in \Ulam$, такое что 
\[
...
\[
> K_2 - \frac{1}{\lamCQ}\log\priorh[\h_1] + \frac{1}{\lamCQ}\log \priorh[\h_2] + \min_{\h}\KL{\q[\teta_1]}{\prior} 
\]
\[
...
\[
   \Loss = \E_{\q} \log \LL - \KL{\q}{\prior}.
\]
При этом на верхнем уровне оптимизации мы ищем те гиперпараметры $\h$, при которых параметры будут давать максимум правдоподобия:
\[
