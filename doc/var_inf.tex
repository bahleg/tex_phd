...
\label{eq:proba_softmax}
	\mathbf{f}_\textnormal{|V|-1} = \frac{\mathbf{f}_{|V|-2}(\dots \mathbf{f}_1(\mathbf{x}))}{\sum_{c=1}^R \textnormal{exp}\bigl( {f}[c]_{|V|-2}(\dots \mathbf{f}_1(\mathbf{x})) \bigr)},
\end{equation}
...
\[
	\textnormal{log} p({y}|\mathbf{x}, \mathbf{w}) =  \textnormal{log}~{f}^{y}_{|V|-1} (\mathbf{f}_{|V-2|}(\dots \mathbf{f}_1(\mathbf{x}))).
\]
