...
\end{enumerate}
В данном разделе рассматривается второй вариант. Будем полагать, что структура  $\boldsymbol{\Gamma}$  модели глубокого обучения $\mathbf{f}$  и метапараметры $\lam$ определены однозначно:
\[
...
\begin{equation}
\label{eq:var_inf_prior}	
p(\mathbf{w}|\mathbf{h}) =  (2\pi) ^{-\frac{n}{2}} |\mathbf{A}|^{\frac{1}{2}} \textnormal{exp} (-\frac{1}{2}\mathbf{w}^\mathsf{T}\mathbf{A}\mathbf{w}).
\end{equation}

Обоснованность модели~\eqref{eq:model_evidence} в этом примере вычисляется аналитически~\cite{hyperopt}:
\begin{equation}
\label{eq:ground}
...
\end{equation}
где $\hat{\mathbf{w}}$ --- значение наиболее вероятных~\eqref{eq:var_inf_posterior} параметров модели:
\[
...
\]
$\mathbf{H}$ --- гессиан минус функции потерь $(-L)$ модели:
\[
...
\label{eq:proba_softmax}
	\mathbf{f}_\textnormal{|V|-1} = \frac{\mathbf{f}_{|V|-2}(\dots \mathbf{f}_1(\mathbf{x}))}{\sum_{c=1}^R \textnormal{exp}\bigl( \mathbf{f}_{|V|-2}[c](\dots \mathbf{f}_1(\mathbf{x})) \bigr)},
\end{equation}
где $\mathbf{f}_{|V|-2}[c]$ --- $c$-я компонента функции $\mathbf{f}_{|V|-2}$. Компонента $c$ вектора $\mathbf{f}_{|V|-1}$ определяет вероятность принадлежности объекта $\mathbf{x}$ к классу $c$. Логарифм правдоподобия зависимой переменной аналогично~\eqref{eq:example1} имеет вид
\[
...
\begin{equation} 
\label{eq:var_elbo}
\textnormal{log}~p(\mathbf{y}|\mathbf{X},\mathbf{h})  = \int_{\mathbf{w}} q(\mathbf{w})\textnormal{log}~\frac{p(\y,\w|\X,\h)}{q(\mathbf{w})}d\mathbf{w} + \textnormal{D}_\textnormal{KL}  \bigl(q(\mathbf{w})||p(\mathbf{w}|\mathbf{y}, \mathbf{X}, \mathbf{h})\bigr) \geq	
\end{equation} 
$$
\geq \int_{\mathbf{w}} q(\mathbf{w})\textnormal{log}~\frac{p(\y,\w|\X,\h)}{q(\mathbf{w})}d\mathbf{w} =
$$
...
$$
p(\mathbf{y},\mathbf{w}|\mathbf{X},\mathbf{h}) = p(\mathbf{y}|\mathbf{X},\w)p(\mathbf{w}|\mathbf{h}).
$$
...
Будем рассматривать задачу нахождения вариационной оценки как задачу оптимизации. Пусть задано множество распределений $\mathfrak{Q} =\{q(\mathbf{w})\}$. Сведем задачу нахождения наиболее близкой вариационной нижней оценки интеграла~\eqref{eq:model_evidence} к оптимизации вида
\[
...
Ниже представлены методы получения вариационных нижних оценок~\eqref{eq:var_elbo2} обоснованности~\eqref{eq:model_evidence}. В первом параграфе рассматривается метод, основанный на аппроксимации апостериорного распределения $p( \mathbf{w}|\mathbf{y}, \mathbf{X}, \mathbf{h})$~\eqref{eq:var_inf_posterior} многомерным гауссовым распределением с диагональной матрицей ковариаций. В последующих параграфах рассматриваются методы, основанные на различных модификациях стохастического градиентного спуска. 
...
Пусть априорное распределение $p(\mathbf{w}|\mathbf{h})$~\eqref{eq:var_inf_prior} параметров модели задано как нормальное:
\[
...
\end{defin}
В данной главе будем рассматривать распределения, полученные из нескольких точек старта оптимизации параметров $\w$ модели $\model$. Для удобства будем использовать $L(\w)$ как эквивалентную форму записи $\Loss$ для $\teta = [\w]^\text{T},$ и $T(\w)$  как эквивалентную форму записи $\TLoss$. 

Пусть значения $\mathbf{w}^1, \dots, \mathbf{w}^r$  --- реализации случайной величины из некоторого распределения $q(\mathbf{w})$. Начальная энтропия распределения $q(\mathbf{w})$ соответствует энтропии распределения $q^0(\mathbf{w})$, из которого были порождены начальные приближения оптимизации параметров $\mathbf{w}^1, \dots, \mathbf{w}^r$. Под действием оператора $T$ распределение параметров $\mathbf{w}^1, \dots, \mathbf{w}^r$ изменяется. Для учета энтропии распределений, полученных в ходе оптимизации,
...
\[
	y = \mathbf{f}(\mathbf{w}, \mathbf{x}) + {\varepsilon}, \quad  {\varepsilon} \sim \mathcal{N}({0}, 1), \mathbf{f} \in {M}.
\]

В качестве множества моделей $M$ были рассмотрены  нейросети с одним скрытым слоем и softplus-функцией активации:
\begin{equation}
\label{eq:model}
	\mathbf{f}(\mathbf{w}, \mathbf{x}) =   (\mathbf{w}_0^{1,2})^{\T}  \textbf{softplus}\bigl((\mathbf{w}_0^{0,1})^{\T} \x  \bigr)  ,
\end{equation}
где $\mathbf{w}_0^{0,1}  \in \mathbb{R}^{n\times n_1}$ --- матрица параметров скрытого слоя нейросети, $\mathbf{w}_0^{1,2} \in \mathbb{R}^{n_1\times 1}$ --- матрица параметров выходного слоя нейросети, $n_1$ --- размер скрытого слоя,  {$\textbf{softplus}(\x) = \textbf{log}\bigl(1+\textbf{exp}(\x)\bigr)$}.
...
$$\boldsymbol{\sigma}(\x) = \bigl(1+\textbf{exp}(-\x)\bigr)^{-1}.$$
