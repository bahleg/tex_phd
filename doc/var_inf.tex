...
\begin{enumerate}
\item Модели $\mathbf{f}$ описываются общим параметрическим семействов моделей глубокого обучения $\mathfrak{F}$, т.е. имеют общий граф описания моделей $(V,E)$, общее пространство параметров $\mathbb{W}$ и пространство структур $\Gb$. При таком подходе сравнение сложности различных моделей является  адекватным, т.к. они определены на общем пространстве структур $\Gb$ и параметров $\mathbb{W}$. Возможно сравнение как статистической сложности модели, так и структурной. TODO 
\item Модели $\mathbf{f}$  не описываются общим параметрическим семейством. В данном случае однозначно определить структурную сложность модели нельзя , поэтому будем полагать, что структуры модели определены однозначно, а априорнные распределения $p(\boldsymbol{\Gamma})$ структур моделей не учитываются при сравнении статистической сложности моделей.
\end{enumerate}
